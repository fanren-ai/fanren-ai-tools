...
import { type Tool, type Category } from "@/data/tools";
...
export default function HomeClient({
  ...
  // TOP 榜：全站优先用后台自定义编排（topRankingIds），否则取 HOT；分类取该类前 10
  const topTools = useMemo(() => {
    if (topCat === "all") {
      const ids = config.topRankingIds;
      if (ids && ids.length) {
        const byId = new Map(allTools.map((t) => [t.id, t]));
        const curated = ids.map((id) => byId.get(id)).filter(Boolean) as Tool[];
        ...
        <div className="px-6 py-8 max-w-6xl mx-auto">
          ...
          {isAllPreview && (
            <div className="mb-10">
              ...
              {/* 热搜词 */}
              {config.hotKeywords.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap mb-8">
                  <span className="text-xs" style={{ color: "var(--text-secondary)" }}>
                    🔥 热搜：
                  </span>
                  {config.hotKeywords.map((kw) => (
                    <button
                      key={kw}
                      onClick={() => {
                        setSearch(kw);
