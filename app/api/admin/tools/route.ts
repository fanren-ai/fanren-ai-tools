...
import { getCatalog, addTool, findToolCategory } from "@/lib/toolsStore";
...
export async function GET(req: NextRequest) {
  ...
  const toolCat = (id: string) => findToolCategory(categories, id);

  let list = allTools.filter((t) => {
    if (cat) {
      const c = toolCat(t.id);
      if (!c || c.id !== cat) return false;
    }
...
export async function POST(req: NextRequest) {
  ...
  const catId = (body.category ?? "").toString().trim();
  if (!name || !/^https?:\/\//i.test(url) || !catId) {
    return NextResponse.json(
      { ok: false, error: "名称、合法官网链接、分类为必填" },
      { status: 400 }
    );
  }
  const tool: Tool = {
    ...
    sub: body.sub ? String(body.sub) : undefined,
    hot: !!body.hot,
    isNew: !!body.isNew,
  };
  const added = await addTool(catId, tool);
  if (!added) return NextResponse.json({ ok: false, error: "分类不存在" }, { status: 400 });

  revalidatePath("/");
  revalidatePath(`/tool/${tool.id}`);
